 ...
 * 选题看板与复盘报告共用，避免重复实现。
 */
export default function AccountFilter({ accounts, selected, onChange, widthClass = "w-[16.5rem]" }: {
  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        className={`border border-hairline bg-card px-3 py-2 text-sm flex items-center gap-2 hover:border-accent transition-colors ${widthClass} justify-between`}
      >
        <span className={`flex-1 min-w-0 truncate text-left ${selected.length > 0 ? "text-ink" : "text-muted"}`}>{label}</span>
      ...
      {open && (
        <div className="absolute top-full right-0 mt-1 bg-card border border-hairline shadow-lg z-20 w-max min-w-full max-h-60 overflow-y-auto">
          <div
          ...
          {accounts?.map((a) => (
            ...
            >
              ...
              {a.mainColor && <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: a.mainColor }} />}
              <span className="whitespace-nowrap pr-2">{a.accountName}</span>
            </div>
          ))}
        </div>
      )}
